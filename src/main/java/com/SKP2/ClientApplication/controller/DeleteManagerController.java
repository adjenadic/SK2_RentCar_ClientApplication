        ...
        btnDelete.addActionListener(event -> {
            try {
                if (MainFrame.getInstance().getUserService().deleteManager(Long.parseLong(tfID.getText()))) {
                    JOptionPane.showMessageDialog(null, "Manager " + tfID.getText() + " successfully deleted.", "Operation successful", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    JOptionPane.showMessageDialog(null, "Manager " + tfID.getText() + " has not been deleted due to an error.", "Operation unsuccessful", JOptionPane.ERROR_MESSAGE);
                }
